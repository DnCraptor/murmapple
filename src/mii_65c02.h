...
typedef struct mii_cpu_t {
	 ...
	 * cpu_D is the 'data' register, used for 16bit operations
	 * cpu_P is the 'pointer' register, used for 16bit addressing
	 * This is not set hard in stone, but typically addressing modes that
	 * are 'indirect' and load from memory will set cpu_P and read in cpu_D
	 * so the opcode doesn't have to handle s.data at all */
	uint16_t 	cpu_D, cpu_P;

#ifdef MII_PACK_P
	union {
		struct {
			uint8_t 	C:1, Z:1, I:1, D:1, B:1, cpu_R:1, V:1, N:1;
		};
		uint8_t 	P;
	}			P;
#else
